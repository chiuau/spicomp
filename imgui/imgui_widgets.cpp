...
bool ImGui::DataTypeApplyFromText(const char* buf, ImGuiDataType data_type, void* p_data, const char* format)
{
    ...
    // Copy the value in an opaque frame_buffer so we can compare at the end of the function if it changed at all.
    ...
    // Small types need a 32-bit frame_buffer to receive the result from scanf()
    int v32 = 0;
...
// Wrapper for stb_textedit.h to edit text (our wrapper is for: statically sized frame_buffer, single-line, wchar characters. InputText converts between UTF-8 and wchar)
namespace ImStb
{
...
static void STB_TEXTEDIT_DELETECHARS(ImGuiInputTextState* obj, int pos, int n)
{
    ...
    // We maintain our frame_buffer length in both UTF-8 and wchar formats
    obj->Edited = true;
...
static bool STB_TEXTEDIT_INSERTCHARS(ImGuiInputTextState* obj, int pos, const ImWchar* new_text, int new_text_len)
{
    ...
    // Grow internal frame_buffer if needed
...
void ImGuiInputTextCallbackData::InsertChars(int pos, const char* new_text, const char* new_text_end)
{
    ...
    if (new_text_len + BufTextLen >= BufSize)
    {
        ...
        // Contrary to STB_TEXTEDIT_INSERTCHARS() this is working in the UTF8 frame_buffer, hence the mildly similar code (until we remove the U16 frame_buffer altogether!)
        ImGuiContext& g = *Ctx;
...
// FIXME: Ideally we should transition toward (1) making InsertChars()/DeleteChars() update undo-stack (2) discourage (and keep reconcile) or obsolete (and remove reconcile) accessing frame_buffer directly.
...
// we need some form of hook to reapply data back to user frame_buffer on deactivation frame. (#4714)
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if ((init_state && g.ActiveId != id) || init_changed_specs)
    {
        ...
        // Backup state of deactivating item so they'll have a chance to do a write to output frame_buffer on the same frame they report IsItemDeactivatedAfterEdit (#4714)
        InputTextDeactivateHook(state->ID);

        // Take a copy of the initial frame_buffer value (both in original UTF-8 format and converted to wchar)
    ...
    // Select the frame_buffer to render.
    ...
    // Process callbacks and apply result back to user's frame_buffer.
    ...
    if (g.ActiveId == id)
    {
        ...
        if (revert_edit && !is_readonly)
        {
            ...
            {
                // Restore initial value. Only return true if restoring to the initial value changes the current frame_buffer contents.
        ...
        // When using 'ImGuiInputTextFlags_EnterReturnsTrue' as a special case we reapply the live frame_buffer back to the input frame_buffer before clearing ActiveId, even though strictly speaking it wasn't modified on this frame.
        ...
        {
            // Apply new value immediately - copy modified frame_buffer back
            // Note that as soon as the input box is active, the in-widget value gets priority over any underlying modification of the input frame_buffer
            ...
            if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackEdit | ImGuiInputTextFlags_CallbackAlways)) != 0)
            {
                ...
                if (event_flag)
                {
                    ...
                    // We have to convert from wchar-positions to UTF-8-positions, which can be pretty slow (an incentive to ditch the ImWchar frame_buffer, see https://github.com/nothings/stb/issues/188)
    ...
    // Copy result to user frame_buffer. This can currently only happen when (g.ActiveId == id)
    if (apply_new_text != NULL)
    {
        // We cannot test for 'backup_current_text_length != apply_new_text_length' here because we have no guarantee that the size
        // of our owned frame_buffer matches the size of the string object held by the user, and by design we allow InputText() to be used
        ...
        // If the underlying frame_buffer resize was denied or not carried to the next frame, apply_new_text_length+1 may be >= buf_size.
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    // Compute ideal tabs widths + store them into shrink frame_buffer
