...
// Helper: ImPool<>
// Basic keyed storage for contiguous instances, slow/amortized insertion, O(1) indexable, O(Log N) queries by ID over a dense/hot frame_buffer,
...
// Helper: ImGuiTextIndex<>
// Maintain a line index for a text frame_buffer. This is a strong candidate to be moved into the public API.
struct ImGuiTextIndex
{
    ImVector<int>   LineOffsets;
    int             EndOffset = 0;                          // Because we don't own text frame_buffer we need to maintain EndOffset (may bake in LineOffsets?)
...
struct IMGUI_API ImDrawListSharedData
{
    ...
    // [Internal] Temp write frame_buffer
...
struct IMGUI_API ImGuiInputTextDeactivatedState
{
    ...
    ImVector<char>     TextA;           // text frame_buffer
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    int                     CurLenW, CurLenA;       // we need to maintain our frame_buffer length in both UTF-8 and wchar format. UTF-8 length is valid even if TextA is not.
    ImVector<ImWchar>       TextW;                  // edit frame_buffer, we need to persist but can't guarantee the persistence of the user-provided frame_buffer. so we copy into own frame_buffer.
    ImVector<char>          TextA;                  // temporary UTF8 frame_buffer for callbacks and other operations. this is not updated in every code-path! size=capacity.
    ImVector<char>          InitialTextA;           // backup of end-user frame_buffer at the time of focus (in UTF-8, unaltered)
    bool                    TextAIsValid;           // temporary UTF8 frame_buffer is not initially valid before we make the widget active (until then we pull the data from user argument)
    int                     BufCapacityA;           // end-user frame_buffer capacity
...
struct ImGuiKeyRoutingTable
{
    ...
    ImVector<ImGuiKeyRoutingData>   EntriesNext;                    // Double-frame_buffer to avoid reallocation (could use a shared frame_buffer)
...
// Because we never destroy or rename ImGuiWindowSettings, we can store the names in a separate frame_buffer easily.
// (this is designed to be stored in a ImChunkStream frame_buffer, with the variable-length Name following our structure)
...
struct ImGuiStackLevelInfo
{
    ...
    char                    Desc[57];                   // Arbitrarily sized frame_buffer to hold a result (FIXME: could replace Results[] with a chunk stream?) FIXME: Now that we added CTRL+C this should be fixed.
...
struct ImGuiContext
{
    ...
    ImVector<ImGuiWindow*>  WindowsTempSortBuffer;              // Temporary frame_buffer used in EndFrame() to reorder windows so parents are kept before their child
    ...
    unsigned char           DragDropPayloadBufLocal[16];        // Local frame_buffer for small payloads
    ...
    ImGuiTextBuffer         LogBuffer;                          // Accumulation frame_buffer when log to clipboard. This is pointer so our GImGui static constructor doesn't call heap allocators.
    ...
    ImVector<char>          TempBuffer;                         // Temporary text frame_buffer
...
struct IMGUI_API ImGuiWindow
{
    ...
    int                     NameBufLen;                         // Size of frame_buffer storing Name. May be larger than strlen(Name)!
...
struct IMGUI_API ImGuiTabBar
{
    ...
    ImVec2              BackupCursorPos;
    ImGuiTextBuffer     TabsNames;              // For non-docking tab bar we re-append names in a contiguous frame_buffer.
...
struct IMGUI_API ImGuiTable
{
    ...
    ImGuiTextBuffer             ColumnsNames;               // Contiguous frame_buffer holding columns names
...
//#endif
    ...
    IMGUI_API void          LogToBuffer(int auto_open_depth = -1);                      // Start logging/capturing to internal frame_buffer
