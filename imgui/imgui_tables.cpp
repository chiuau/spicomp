...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    // Setup memory frame_buffer (clear data if columns count changed)
    ...
    if (table->RefScale != 0.0f && table->RefScale != new_ref_scale_unit)
    {
        ...
        //IMGUI_DEBUG_PRINT("[table] %08X RefScaleUnit %.3f -> %.3f, scaling width by %.3f\n", table->ID, table->RefScaleUnit, new_ref_scale_unit, map_to_win_scale_factor);
        for (int n = 0; n < columns_count; n++)
...
{
    // Allocate single frame_buffer for our arrays
...
void ImGui::TableSetupColumn(const char* label, ImGuiTableColumnFlags flags, float init_width_or_weight, ImGuiID user_id)
{
    ...
    // Store name (append with zero-terminator in contiguous frame_buffer)
    column->NameOffset = -1;
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
    ...
    // Use a reusable temp frame_buffer for the merge masks as they are dynamically sized.
...
// Instead, we do a preemptive overwrite of clipping rectangle _without_ altering the command-frame_buffer and let
